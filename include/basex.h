...
OutputIterator bxdecode(InputIterator beg, InputIterator end, OutputIterator result, const std::string &digits)
{
    // TODO generate decmap at compile time with TMP
    int8_t decmap[256];
...
inline InputIterator lastn(InputIterator beg, InputIterator end, typename std::iterator_traits<InputIterator>::difference_type n, std::input_iterator_tag)
{
    auto result = beg;
    while (n--) { ++beg; }
...
template <typename InputIterator>
inline InputIterator lastn(InputIterator beg, InputIterator end, typename std::iterator_traits<InputIterator>::difference_type n)
{
    return lastn(beg, end, n, typename std::iterator_traits<InputIterator>::iterator_category());
}


template <typename InputIterator, typename OutputIterator>
OutputIterator encode(InputIterator beg, InputIterator end, OutputIterator result)
{
    auto size = std::distance(beg, end);
    auto nremainder = size % 3;
    end = lastn(beg, end, nremainder);
...
// istream_iterator and istreambuf_iterator should be handled carefully
template <typename InputIterator, typename OutputIterator>
inline OutputIterator buffered_codec(InputIterator beg, InputIterator end, OutputIterator result, bool is_encode = true)
{
    ...
    while (beg != end) {
        ...
        if (is_encode) {
            result = encode(buffer.cbegin(), buffer.cbegin() + i, result);
        } else {
            if (i & 3 && !(i &= ~3U)) {
                break;  // trailing '\n' and other garbages are ignored
            }
            result = decode(buffer.cbegin(), buffer.cbegin() + i, result);
        }
    }
    return result;
}
...
inline OutputIterator encode(std::istream_iterator<T> beg, std::istream_iterator<T> end, OutputIterator result)
{
    return buffered_codec(beg, end, result);
}

template <typename T, typename OutputIterator>
inline OutputIterator decode(std::istream_iterator<T> beg, std::istream_iterator<T> end, OutputIterator result)
{
    return buffered_codec(beg, end, result, false);
}

template <typename T, typename OutputIterator>
inline OutputIterator encode(std::istreambuf_iterator<T> beg, std::istreambuf_iterator<T> end, OutputIterator result)
{
    return buffered_codec(beg, end, result);
}

template <typename T, typename OutputIterator>
inline OutputIterator decode(std::istreambuf_iterator<T> beg, std::istreambuf_iterator<T> end, OutputIterator result)
{
    return buffered_codec(beg, end, result, false);
}
}  // namespace b64
