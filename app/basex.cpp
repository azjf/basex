...
using std::array, std::as_const, std::map, std::string;
using std::cin, std::cout, std::cerr, std::endl, std::ios, std::ifstream, std::istreambuf_iterator, std::ostreambuf_iterator, std::filesystem::is_regular_file;
...
Mandatory arguments to long options are mandatory for short options too)"""");
    // clang-format off
    generic.add_options()
        ("algorithm,a", po::value<string>(&algo)->default_value("b64"), "algorithm: b64 (RFC 4648), b58 (Bitcoin), or codec digits for base convertion")
...
int main(int argc, char **argv)
{
    ...
    ifstream f;
    if (!input_file.empty()) {
        auto err = [&input_file = as_const(input_file)](const string &msg) {
            cerr << "basex: " << input_file << ": " << msg << endl;
            return 1;
        };
        if (!is_regular_file(input_file)) { return err("No such file"); }
        f = ifstream(input_file, ios::binary);
    ...
    const map<string, array<decltype(result) (*)(decltype(beg), decltype(end), decltype(result)), 2>> funcs{{"b64", {b64::encode, b64::decode}},
